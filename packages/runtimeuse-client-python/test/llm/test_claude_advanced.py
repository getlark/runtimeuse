"""Advanced LLM integration tests for the Claude agent.
...
class TestClaudePrePostCommands:
    async def test_pre_and_post_commands_run(self, claude_ws_url: str):
...
class TestClaudeArtifactsS3:
    async def test_artifact_uploaded_to_s3(
        self, claude_ws_url: str, s3_client, s3_test_bucket: str
    ):
        run_id = str(uuid4())
        ...
        result = await client.query(
            prompt=(
                ...
                "'hello from claude'. Use the bash tool."
            ),
            options=QueryOptions(
        ...
        assert b"hello from claude" in body

        s3_client.delete_object(Bucket=s3_test_bucket, Key=artifact_key)


class TestClaudeFullLifecycle:
